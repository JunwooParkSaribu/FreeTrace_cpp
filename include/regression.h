#pragma once // Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-12 10:00
#include <vector>
#include <cmath>
#include <array>
...
// Uses double precision to match Python (scipy lstsq returns float64)
std::vector<std::array<double, 6>> pack_vars(const float* vars, int len_img);
...
UnpackResult unpack_coefs(const std::vector<std::array<double, 6>>& coefs, int win_w, int win_h);
...
// Returns: [nb_imgs, 6] fitted coefficients (a, b, c, d, e, f) in double
std::vector<std::array<double, 6>> guo_algorithm( // Modified by Claude (claude-opus-4-6, Anthropic AI) - 2026-03-12 10:00
