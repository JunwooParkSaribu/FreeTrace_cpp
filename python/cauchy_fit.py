...
# Refined H grid (step = 0.125): adds 0.125, 0.375, 0.625, 0.875.
_AVAILABLE_H_GRID = (0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875)


def _load_cov_table(H: float) -> dict:
    """Load the tabulated (S_rr, S_gg, S_rg) cov table for the given H.

    Snaps H to the nearest tabulated value in _AVAILABLE_H_GRID = {0.125, 0.25,
    0.375, 0.5, 0.625, 0.75, 0.875}. Tables are bundled next to cauchy_fit.py as
    cauchy_neff_cov_H<NNNN>.npz where NNNN = round(H*1000) zero-padded to 4 digits
    (e.g. H=0.125 -> H0125, H=0.25 -> H0250).
    """
    import os as _os
    ...
    h_tag = f"{int(round(H_snap * 1000)):04d}"
    here = _os.path.dirname(_os.path.abspath(__file__))
    path = _os.path.join(here, f"cauchy_neff_cov_H{h_tag}.npz")
    if not _os.path.exists(path):
        raise RuntimeError(
            f"cov table not found: {path} (snap H={H_snap} for fitted H={H:.3g})"
        )
    d = np.load(path)
